...
from lox.utils import RegexEqual
...
class Scanner():
    ...
    def add_number(self) -> None:
        ...
        if self.source[self.current] == "." and self.is_digit(self.source[self.current + 1]):
            self.current += 1
            while self.is_digit(self.source[self.current]):
                self.current += 1
    ...
    def add_string(self) -> None:
        ...
        str_value = self.source[self.start + 1:self.current - 1] # Trim the surrounding quotes
